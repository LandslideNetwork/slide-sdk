...
import (
	...
	"github.com/landslidenetwork/slide-sdk/utils/evm/validators/state/interfaces"
...
func NewState(db dbm.DB) (interfaces.State, error) {
	s := &state{
...
func (s *state) WriteState() error {
	...
	for vID, updateStatus := range s.updatedData {
		...
		case updatedStatus:
			...
			dataBytes, err := vdrCodec.Marshal(data)
			if err != nil {
				return err
			}
			if err := batch.Set(vID[:], dataBytes); err != nil {
				return err
			}
		case deletedStatus:
...
func parseValidatorData(bytes []byte, data *validatorData) error {
	if len(bytes) != 0 {
		if err := vdrCodec.Unmarshal(bytes, data); err != nil {
			return err
		}
	}
	return nil
...
func (s *state) loadFromDisk() error {
	...
	if err != nil {
		return fmt.Errorf("failed to create db iterator: %w", err)
	}
	defer it.Close()
