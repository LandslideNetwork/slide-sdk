...
type BlockExecutor struct {
	...
	// blockStore for the pruning functions.
...
// CreateProposalBlock calls statetypes.MakeBlock with evidence from the evpool
...
func (blockExec *BlockExecutor) ProcessProposal(
	...
	resp, err := blockExec.proxyApp.ProcessProposal(context.Background(), &abci.RequestProcessProposal{
...
func (blockExec *BlockExecutor) ApplyBlock(
	...
	abciResponse, err := blockExec.proxyApp.FinalizeBlock(context.Background(), &abci.RequestFinalizeBlock{
		Hash:               block.Hash(),
		NextValidatorsHash: block.NextValidatorsHash,
		ProposerAddress:    block.ProposerAddress,
		Height:             block.Height,
...
func (blockExec *BlockExecutor) Commit(
	...
	res, err := blockExec.proxyApp.Commit(context.Background())
	if err != nil {
...
func buildExtendedCommitInfoFromStore(ec *types.ExtendedCommit, store statetypes.Store, initialHeight int64, ap types.ABCIParams) abci.ExtendedCommitInfo {
...
func updateState(
	...
	if abciResponse.ConsensusParamUpdates != nil {
		// NOTE: must not mutate statetypes.ConsensusParams
	...
	// It will be filled on statetypes.Save.
	return statetypes.State{
		...
		LastResultsHash:                  statetypes.TxResultsHash(abciResponse.TxResults),
		AppHash:                          nil,
	}, nil
}
...
func ExecCommitBlock(
	...
	resp, err := appConnConsensus.FinalizeBlock(context.Background(), &abci.RequestFinalizeBlock{
		Hash:               block.Hash(),
		NextValidatorsHash: block.NextValidatorsHash,
		ProposerAddress:    block.ProposerAddress,
		Height:             block.Height,
	...
	// Commit block
	_, err = appConnConsensus.Commit(context.Background())
	if err != nil {
...
func (blockExec *BlockExecutor) pruneBlocks(retainHeight int64, state statetypes.State) (uint64, error) {
