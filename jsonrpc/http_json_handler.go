...
func makeJSONRPCHandler(funcMap map[string]*RPCFunc, logger log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		logger.Info("request", "body", string(b))
		...
		logger.Info("process requests", "qty", len(requests))
		for _, request := range requests {
			...
			// real handler argument count and defined for RPC is mismatched
			if len(rpcFunc.args)-1 != len(rpcFunc.argNames) {
				responses = append(
					responses,
					types.RPCInternalError(request.ID, fmt.Errorf("rpc function expects %v parameters, defined %v (%v)",
						len(rpcFunc.args)-1, len(rpcFunc.argNames), rpcFunc.argNames)),
				)
				cache = false
				continue
			}
